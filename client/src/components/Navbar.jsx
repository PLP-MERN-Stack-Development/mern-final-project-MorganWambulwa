...
import { HeartHandshake, Menu, X, LogOut, LayoutDashboard, Phone, Facebook, Twitter, Instagram, Youtube } from "lucide-react";
    ...
    <div className="fixed top-0 w-full z-50">
      ...
      <nav className="bg-white/95 backdrop-blur-md border-b border-gray-100 shadow-sm transition-all duration-300">
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <button onClick={() => scrollToSection("home")} className="flex items-center gap-2 text-2xl font-bold text-emerald-600 hover:text-emerald-700 transition-colors">
          ...
          <div className="hidden md:flex items-center gap-4">
            {user ? (
              <div className="flex items-center gap-4">
                ...
                <Button variant="ghost" size="icon" onClick={handleSignOut} className="text-gray-400 hover:text-red-600 hover:bg-red-50 h-9 w-9" title="Sign Out">
              ...
              <>
                <Link to="/auth">
                  <Button variant="ghost" className="text-gray-600 hover:text-emerald-700 hover:bg-emerald-50 font-medium">Sign In</Button>
                </Link>
                <Link to="/auth">
                  <Button className="bg-gradient-hero shadow-md hover:shadow-lg transition-all">Get Started</Button>
                </Link>
              </>
            )}
          </div>
          ...
          <div className="md:hidden bg-white border-t border-gray-100 shadow-xl absolute w-full left-0 animate-in slide-in-from-top-5 duration-200">
            <div className="p-4 space-y-4">
              <div className="space-y-2 border-b border-gray-100 pb-4">
                <button onClick={() => scrollToSection("home")} className="block w-full text-left py-2 px-3 rounded-md hover:bg-gray-50 font-medium text-gray-700">Home</button>
                <Link to="/about" onClick={() => setIsMobileMenuOpen(false)} className="block w-full text-left py-2 px-3 rounded-md hover:bg-gray-50 font-medium text-gray-700">About Us</Link>
                <button onClick={() => scrollToSection("impact")} className="block w-full text-left py-2 px-3 rounded-md hover:bg-gray-50 font-medium text-gray-700">Impact</button>
              </div>
              <div className="pt-2 flex flex-col gap-3">
                {user ? (
                  <>
                    ...
                    <Button variant="outline" className="w-full border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700 justify-start" onClick={handleSignOut}>
                  ...
                  <>
                    <Link to="/auth" onClick={() => setIsMobileMenuOpen(false)}>
                      <Button variant="outline" className="w-full justify-center border-gray-300 text-gray-700">Sign In</Button>
                    </Link>
                    <Link to="/auth" onClick={() => setIsMobileMenuOpen(false)}>
                      <Button className="w-full bg-gradient-hero justify-center shadow-md">Get Started</Button>
                    </Link>
                  </>
                )}
              </div>
